...
window.saveAndPrint = async function() {
  ...
  // If in reprint mode, just print without saving
  if (isReprintMode && reprintInvoiceId === invNum) {
    btn.textContent = '🖨️ Printing...';
    printInvoice(invNum);
    
    setTimeout(() => {
      // Exit reprint mode and reset form
      isReprintMode = false; 
...
window.reprintInvoice = async function(invId) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === invId) {
      ...
      if (itemsJSON.length > 0) {
        itemsJSON.forEach(item => {
          ...
          tr.innerHTML = `
            ...
            <td><input type="number" class="item-qty" value="${item.qty || 1}" aria-label="Item quantity"></td>
            <td><input type="number" class="item-price" value="${item.price || 0}" aria-label="Item unit price"></td>
            <td><span class="amount-display" aria-label="Item amount">${((item.qty || 0) * (item.price || 0)).toFixed(2)}</span></td>
          `;
          tbody.appendChild(tr);
        });
      } else {
      ...
      // Set reprint mode BEFORE showing toast
      isReprintMode = true; 
